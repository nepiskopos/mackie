...
| No runtime quality gate | LLM-as-judge exists only in `scripts/eval.py`; the chatbot trusts the agent LLM to apply preferences and avoid duplicates with no verification | Add async judge calls after `save_post` to score drafts against brand voice, and before surfacing suggestions to catch semantic duplicates; run with a cheap fast model to keep latency low |
